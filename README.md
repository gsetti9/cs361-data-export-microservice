...
file, the microservice processes it, and writes back a response file.
...
- `output_path`: the file path where the exported file will be saved (e.g. `"my_export.csv"` or 
  `"exports/portfolio.json"`). The file will be created automatically if it doesn't exist. If the 
